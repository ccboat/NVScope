...
class ScriptBase:
    ...
    def load_excluded_functions(
        ...
        for f_sink in self.sinks:
            ...
            if self.project.arch.name == 'MIPS32':
                functions = []
                for ad, fu in self.project.kb.functions.items():
                    if fu.name == f_sink.name:
                        functions.append(fu)
                cg = self.project.kb.functions.callgraph
                for f in functions:
                    if len(set(cg.predecessors(f.addr))) > 0:
                        sink_func = f
    ...
    def load_sinks(custom_sink=None, arg_pos=None, category=None) -> List[Sink]:
        ...
        sinks += VULN_TYPES["overflow"]

        setters = ['setenv', 'script_setenv', 'nvram_set', 'acosNvramConfig_set',
                                          'acosNvramConfig_write', 'j_nvram_set', 'nvram_set_str', 'bcm_nvram_set',
                                          'envram_set', 'wlcsm_nvram_set', 'dni_nvram_set', 'PIT_nvram_set',
    ...
    def analyze(self):
        ...
        try:
            for sink, sink_function in self.get_sink_callsites(self.sinks):
                ...
                fname = sink_function.name
                ida_path = self.result_path / 'ida'
                ida_path.mkdir(parents=True, exist_ok=True)
                ida_file = ida_path / fname
                with open(ida_file, 'w+') as f:
                    for t in traces:
                        for x in t.callsites:
                            f.write(hex(x.caller_func_addr) + ',' + hex(x.callee_func_addr))
                            f.write(' ')
                        f.write('\n')
        ...
        for call_idx, callsite in enumerate(trace.callsites):
            ...
            try:
                ...
                for w in handler.white_list:
                    if w.code_loc.ins_addr in addrs:
