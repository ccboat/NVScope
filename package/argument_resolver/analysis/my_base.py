...
class ScriptBase:
    ...
    def load_excluded_functions(
        ...
        for f_sink in self.sinks:
            ...
            for addr,func in self.project.kb.functions.items():
                if func.name == f_sink.name and func.calling_convention:
                    sink_func = func
    ...
    def get_sink_callsites(self, sinks: List[Sink]) -> List[Tuple[Sink, Function]]:
        ...
        for sink in sinks:
            ...
            for addr,func in self.project.kb.functions.items():
                if func.name == sink.name and func.calling_convention:
                    function = func
    ...
    def gen_traces_to_sink(
        ...
        if not self.forward_trace:
            ...
            for idx, trace in enumerate(
                ...
                with open(self.result_path / 'my_trace','a+') as ff:
                    ff.write(str(len(white_list)))
                    ff.write('\n')
        ...
        for call_idx, callsite in enumerate(trace.callsites):
            ...
            try:
                ...
                if self.project.arch.name == 'MIPS32':
                    for w in handler.white_list:
                        if not w.code_loc.ins_addr:
                            after_whitelist.append(w)
                        elif w.code_loc.ins_addr - 4 in addrs:
                            after_whitelist.append(w)
                        else:
                            del_list.append(w)
                else:
                    for w in handler.white_list:
                        if not w.code_loc.ins_addr:
                            after_whitelist.append(w)
                        elif w.code_loc.ins_addr in addrs:
                            after_whitelist.append(w)
                        else:
                            del_list.append(w)

                if len(after_whitelist) == 0:
                    after_whitelist = handler.white_list

            except:
                after_whitelist = handler.white_list

            handler.white_list = after_whitelist
