...
from argument_resolver.my_handlers.base import HandlerBase
from argument_resolver.my_handlers.local_handler import LocalHandler
...
from argument_resolver.analysis.env_base import ScriptBase, default_parser
...
class MangoAnalysis(ScriptBase):
    ...
    def get_sources_from_closure(closure) -> Dict:
        ...
        print(closure.handler._sink_atoms)
    ...
    def get_key_from_closure(self, closure, key_atom):
        try:
            key_str = Utils.get_strings_from_pointers(
                closure.sink_trace.arg_vals[key_atom],
                closure.sink_trace.state,
                closure.sink_trace.code_loc,
            )
            print('keystr')
            print(key_str)
            if 'TOP' in str(key_str):
                return 'TOP'

            constant = closure.sink_trace.constant_data[key_atom]
            print(constant)
            if len(constant)==1:
                constant = str(Utils.bytes_from_int(constant[0]).decode("latin-1"))
                print('constant')
                print(constant)
                return constant
            else:
                ...
                for i in range(len(constant)):
                    conslist.append(str(Utils.bytes_from_int(constant[i]).decode("latin-1")))
                print('constant')
                print(conslist)
                return str(conslist)
        except:
    ...
    def save_closure(self, sink, defn, closure, key_atom):
        ...
        if sink.name in setters:
            key_src = self.get_key_from_closure(closure, key_atom)
        else:
            key_src = ''
    ...
    def post_analysis(
        ...
        key_atom = 0
        for trace, defns in potential_sinks.items():
            for idx, atoms in enumerate(trace.args_atoms):
                if idx == 0:
                    for atom in atoms:
                        key_atom = atom
                    break
        ...
        '''
        outdict = self.out_dict
        if sink_function.name not in outdict:
            outdict[sink_function.name] = {}

        for trace, defns in potential_sinks.items():
            xaddr = hex(trace.code_loc.ins_addr)
            if xaddr not in outdict[sink_function.name]:
                outdict[sink_function.name][xaddr] = {}

            for idx, atoms in enumerate(trace.args_atoms):
                print(atoms)
                if idx not in outdict[sink_function.name][xaddr]:
                    outdict[sink_function.name][xaddr][idx] = {}
                    outdict[sink_function.name][xaddr][idx]['constant'] = []
                    outdict[sink_function.name][xaddr][idx]['source'] = []
                for atom in atoms:
                    if atom in trace.constant_data:
                        constant = trace.constant_data[atom]
                        if constant is not None:
                            for ci in constant:
                                if ci is not None:
                                    outdict[sink_function.name][xaddr][idx]['constant'].append(
                                        Utils.bytes_from_int(ci).decode("latin-1"))

                print(atom)
                syms = Utils.get_strings_from_pointers(
                    trace.arg_vals[atom],
                    trace.state,
                    trace.code_loc,
                )
                ...
                for s in sym:
                    print(s)
                    if 'TOP' in str(s):
                        outdict[sink_function.name][xaddr][idx]['source'].append(('unknown', str(s)))
                    elif '@' in str(s):
                        for category, funcs in categories.items():
                            for func in funcs:
                                if func in str(s).lower():
                                    outdict[sink_function.name][xaddr][idx]['source'].append((func, str(s)))
                    elif 'ARGV' in str(s):
                        argvs = re.findall(r'ARGV_\d+', str(s))
                        for argv in argvs:
                            outdict[sink_function.name][xaddr][idx]['source'].append(('ARGV', argv))
                    else:
                        try:
                            constant = Utils.bytes_from_int(s).decode("latin-1")
                        except:
                            constant = str(s)
                            outdict[sink_function.name][xaddr][idx]['source'].append(('sym', constant))
                        if constant not in outdict[sink_function.name][xaddr][idx]['constant']:
                            outdict[sink_function.name][xaddr][idx]['constant'].append(constant)
                outdict[sink_function.name][xaddr][idx]['sym'] = str(sym)

        self.out_dict = outdict


        self.bout = open(self.result_path / 'setter_references', 'w')
        self.bout.write(self.bin_path)
        self.bout.write('\n')
        self.bout.write(str(outdict))
        self.bout.write('\n')
        self.bout.close()
        '''
        ...
        for defn, closures in unresolved_closures.items():
            for closure in closures:
                ...
                sources = self.save_closure(sink_function, defn, closure, key_atom)
                if sources:
