...
from argument_resolver.my_handlers import (
    NVRAMHandlers,
    NetworkHandlers,
    StdioHandlers,
    StdlibHandlers,
...
setters = ['setenv', 'script_setenv', 'nvram_set', 'acosNvramConfig_set', 'acosNvramConfig_write',
           'j_nvram_set',
...
class ScriptBase:
    ...
    def load_sinks(custom_sink=None, arg_pos=None, category=None) -> List[Sink]:
        sinks = []
        print('load')
        for s in setters + EXECS:
        #for s in ['nvram_set']:
            sinks = sinks + [Sink(s, [2])]
        print(sinks)
        return sinks
    ...
    def analyze(self):
        ...
        try:
            for sink, sink_function in self.get_sink_callsites(self.sinks):
                try:
                    ...
                    for trace, white_list, trace_idx, total_traces in traces:
                        ...
                        tt = time.time()-sink_start
                        if tt > 12*60*60:
                            print('sink timeout: '+str(sink_function.name))
                            break
            ...
            self.progress.stop()
            self.save_results()
            self.save_env_results(self.out_dict, self.result_path)
    ...
    def save_env_results(self, out_dict, bres_path):
        sets = {}
        execs = {}
        sprtfs = {}
                ...
                for addr in out_dict[setter]:
                    ...
                    if key['constant'] is not None:
                        for k in key['constant']:
                            ...
                            if len(val['constant']) == 0 and len(val['source']) > 0:
        ...
        se_pa = bres_path / 'found_setters'
        with open(se_pa, 'w') as bout:
    ...
    def check_trace_rda(self, trace: CallTrace, sink: Function, sink_atoms: List[Atom]):
        ...
        for call_idx, callsite in enumerate(trace.callsites):
            ...
            caller = callsite.caller_func_addr
            callee = callsite.callee_func_addr
            fname = str(hex(caller)) + '_' + str(hex(callee))

            try:
                ida_path = self.result_path / 'ida_result' / sink.name
                ida_file = ida_path / fname
                print(ida_file)
                with open(ida_file, "r") as f:
                    c = f.readlines()
                    addrs = [int(i.strip(), 16) for i in c]
            except:
                addrs = []
            print('ida addrs')
            print(addrs)
            ...
            timed_out = False
            print('tainting '+str(single_trace.callsites))
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                try:
                    ...
                    print('check time out: '+ str(single_trace.callsites))
                    break
                except Exception as e:
                    print(e)
                    continue
            ...
            print('check trace done: '+str(single_trace.callsites))
            if timed_out:
                continue
            ...
            print('handler whitelist')
            print(handler.white_list)
    ...
    def run_analysis_on_trace(
        ...
        print('run_analysis_on_trace '+str(function.name)+' '+str(function_address))
        ...
        try:
            ...
            print('run analysis rda TIMEOUT')
            rda = None
        except Exception as e:
            print(e)
            rda = None
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            try:
                ...
                print('rda timeout: '+str(subject.content.callsites))
